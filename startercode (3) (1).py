...
def spinWheel(playerNum):
    ...
    if(wheelspin == 'BANKRUPT'):
        print("The result is BANKRUPT. Nice going, now your round total is 0.") #could not figure out why the BANKRUPT and LOSE A TURN ends the script
        players[playerNum]['roundtotal'] = 0
        stillinTurn = False
        print("Ah too bad just be better")
        goodGuess = False
    elif(wheelspin == "LOSE A TURN"):
        print("The wheel landed on LOSE A TURN. Nice try, you'll get em on the next one.")
        stillinTurn = False
        print("Ah that sucks for you")
        goodGuess = False
    else:
...
def guessWord(playerNum):
    ...
    if word == roundWord:
        for i in range (len(roundWord)):
            if word == roundWord:
                print(f"You got the word! The word was {roundWord}")
...
def wofTurn(playerNum):  
    ...
    while stillinTurn:
        ...
        if(choice.strip().upper() == "S"):
            ...
            stillinRound, stillinTurn = guessWord(playerNum)
...
def wofFinalRound():
    ...
    for i in players.keys():
        if players[i]['gametotal'] > players[winplayer]['gametotal']:
            winplayer = i
    print(finalroundtext.format(name = players[winplayer]['name'], bank = players[winplayer]['gametotal']))
    ...
    # finalguesses = []
    firstconsonant = guessletter(input("First consonant guess:"), winplayer)
    secconsonant = guessletter(input("Second consonant guess:"), winplayer)
    thirdconsonant = guessletter(input("Third consonant guess:"), winplayer)
    firstvowel = buyVowel(input("Vowel guess:"))
